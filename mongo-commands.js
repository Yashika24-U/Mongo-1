...
// use dbName - create and switch dbName
// db - currentdb
...
db.books.find({ratings:{$gt:9} }).pretty()

// display all rating <= 7
db.books.find({rating:{$lte:7} }).count()
...
//========================================================
//Projection == inclusion - 1,exclusion - 0
//inclusion - 1
...
//Limit
db.books.find({rating:{$gt:8}},{_id:0,name:1,rating:1}).sort({name:1}).limit(2).pretty()

//skip
db.books.find({rating:{$gt:8}},{_id:0,name:1,rating:1}).sort({rating:-1}).limit(2).skip(2).pretty()
...
//stage1:Iam going to pick that matches status:urgent
...
//5. delete all the books with rating > 8.8
...
db.books.updateOne(
    {_id:ObjectId("64d6524e2e399b861a9d3c0e"),},
    {$set : {language:"Tamil"}}
)

db.books.updateOne(
    {_id:ObjectId("64d6524e2e399b861a9d3c10"),},
    {$set :{language:"Hindi"}}
)
...
db.books.find({rating:{$gt:5}},{name:1,rating:1}).pretty()
